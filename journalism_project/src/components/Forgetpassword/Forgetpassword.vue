...
<div class="registerz">
  ...
  <div class="aggregate">
    ...
    <button class="registerbtn" v-on:click="postreg">完成</button>
  </div>
   <div class="changereg">
  ...
  export default {
    ...
    methods: {
      ...
      getSms: function () {
        ...
        var vm = this
        // 将用户的图文验证码作为参数传递
        getSmsCaptchaf(vm.captcha,vm.phone).then(res => {
      ...
      getshort: function() {
        ...
        var vm = this
         // 将手机号码作为参数传递
         getShortmessagef(vm.phone).then(res => {
            alert(res.data.message)
        })
         // 捕获错误
          .catch(err => {
      ...
      postreg: function () {
        ...
        var vm = this
        // 将用户的图文验证码作为参数传递
        postregisterf(vm.phone,vm.password,vm.smsCaptcha).then(res => {
            alert(res.data.message)
        })
          // 捕获错误
          .catch(err => {
